...
- Phase 6 Step 8 ✅ — Dashboard operator launch path: the `Benchmark Profile Catalog` panel now includes HTMX-backed operator forms that dispatch directly into the existing MCP runtime helpers for `mars.benchmark` and `mars.release`. Operators can select a configured benchmark profile, run a benchmark review, or create a release bundle from Mission Ops and immediately inspect the returned request ID and payload without leaving the dashboard.
