...
- Phase 6 Step 5 ✅ — Config-backed benchmark profile selection: `configs/knowledge_release_policy.toml` now defines named benchmark profiles plus the default profile, `BenchmarkHarness.from_policy_profile()` materializes a harness from that config, and both `mars.benchmark` and `mars.release` accept an optional `benchmark_profile` override so operators can run the same release path against an explicit configured policy lane such as `nasa-esa-mission-review-permissive`.
- Phase 6 Step 6 ✅ — Operator-facing benchmark profile discovery: MCP adapter/server now expose `mars.benchmark.profiles`, which returns the configured default benchmark profile together with all available profile names, policy versions, policy sources, covered metrics, and reference counts. Operators no longer need benchmark profile names out of band before invoking `mars.benchmark` or `mars.release`.
- Phase 6 Step 7 ✅ — Visual benchmark profile catalog: the Mission Ops dashboard now renders a `Benchmark Profile Catalog` panel that polls `/dashboard/fragments/benchmark-profiles` and shows the default profile, total reference count, each configured profile’s version/source, and covered metrics as operator-readable chips. Benchmark policy discovery is now available both through MCP (`mars.benchmark.profiles`) and through the dashboard UI.
- Phase 6 Step 8 ✅ — Dashboard operator launch path: the `Benchmark Profile Catalog` panel now includes HTMX-backed operator forms that dispatch directly into the existing MCP runtime helpers for `mars.benchmark` and `mars.release`. The panel prefills `plan_id` and `simulation_id` from the latest successful correlation chain when runtime state is available, and release results now surface manifest and bundle-path highlights above the raw payload so operators can act on exported artifacts immediately.
...
- Phase 11 Step 6 ✅ — Internal router/mailbox runtime: `NegotiationEnvelope` and `NegotiationMailbox` now back a deterministic in-process `NegotiationRouter`/`NegotiationRuntime` in `src/mars_agent/orchestration/negotiation_runtime.py`; planner-side proposal/review/counter collection helpers are no longer the active execution path. `CentralPlanner` now delegates specialist proposal routing, peer delivery, transcript recording, and counter-proposal delivery to the runtime while keeping `mars.plan` and MCP payloads unchanged.
- Phase 11 Step 7 ✅ — Bounded multi-round convergence: the negotiation runtime now converts bounded counter-proposals into a revised proposal wave and re-runs peer review until convergence or a small fixed round cap is reached. The planner still closes the session and the public planner/MCP contracts remain unchanged, but internal transcripts now show the additional converged proposal/review wave when counters materially revise a proposal.

## Next

- Continue the Phase 11 rollout by deciding whether the next internal step is planner-supervisor-only session control or an explicit transport abstraction for cross-process specialist routing.
